...
<h2 class="font-heading text-2xl font-bold text-gray-900 mb-1">Set new password</h2>
...
<form method="POST" action="{{ route('password.update') }}" class="space-y-5">
    ...
    <div>
        ...
        <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}"
               ...
               class="block w-full rounded-lg border px-3.5 py-2.5 text-sm text-gray-900 transition-colors
                      @error('email') border-red-400 ring-2 ring-red-100 @else border-gray-300 @enderror
                      focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100">
        @error('email')
        <p class="mt-1.5 text-xs text-red-600 flex items-center gap-1"><i data-lucide="alert-circle" class="w-3.5 h-3.5"></i> {{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <input id="password-confirm" type="password" name="password_confirmation" required autocomplete="new-password"
               class="block w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 transition-colors
                      focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100">
    </div>
