...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="font-heading text-xl font-bold text-gray-900">Edit Fee Structure: {{ $fee_structure->name }}</h1>
        <nav class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
            ...
            <a href="{{ route('finance.fee-structures.index') }}" class="hover:text-indigo-600 transition-colors">Fee
                Structures</a>
            <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
            <span class="text-gray-900">Edit Fee Structure</span>
        </nav>
    </div>
    ...
    items: {{ json_encode($fee_structure->items->map(fn($item) => ['fee_type_id' => $item->fee_type_id, 'amount' => $item->amount])) }},
    addItem() {
    ...
    removeItem(index) {
        ...
        <form action="{{ route('finance.fee-structures.update', $fee_structure->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <div class="lg:col-span-2">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $fee_structure->name) }}"
                ...
                <div>
                    ...
                    <select id="session_id" name="session_id" required
                        ...
                        @foreach($sessions as $session)
                            <option
                                value="{{ $session->id }}" {{ old('session_id', $fee_structure->session_id) == $session->id ? 'selected' : '' }}>{{ $session->session_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="class_id" name="class_id" required
                        ...
                        @foreach($classes as $class)
                            <option
                                value="{{ $class->id }}" {{ old('class_id', $fee_structure->class_id) == $class->id ? 'selected' : '' }}>{{ $class->class_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="section_id" name="section_id"
                        ...
                        @foreach($sections as $section)
                            <option
                                value="{{ $section->id }}" {{ old('section_id', $fee_structure->section_id) == $section->id ? 'selected' : '' }}>{{ $section->section_name }}
                ...
                <div>
                    ...
                    <select id="semester_id" name="semester_id"
                        ...
                        @foreach($semesters as $semester)
                            <option
                                value="{{ $semester->id }}" {{ old('semester_id', $fee_structure->semester_id) == $semester->id ? 'selected' : '' }}>{{ $semester->semester_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                <button type="submit"
                    ...
                    <i data-lucide="save" class="w-4 h-4"></i> Update Structure
                </button>
