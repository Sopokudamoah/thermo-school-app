...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="font-heading text-xl font-bold text-gray-900">Edit Discount: {{ $discount->name }}</h1>
        <nav class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
            ...
            <a href="{{ route('finance.discounts.index') }}"
               class="hover:text-indigo-600 transition-colors">Discounts</a>
            <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
            <span class="text-gray-900">Edit Discount</span>
        </nav>
    </div>
    ...
    <div class="bg-white rounded-card shadow-card border border-gray-200 p-6 max-w-2xl">
        <form action="{{ route('finance.discounts.update', $discount->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="space-y-4 mb-6">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">Discount Name <sup
                            class="text-indigo-500">*</sup></label>
                    <input type="text" id="name" name="name" value="{{ old('name', $discount->name) }}"
                           placeholder="e.g. Sibling Discount" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="type" class="block text-sm font-medium text-gray-700 mb-1.5">Discount Type <sup
                                class="text-indigo-500">*</sup></label>
                        <select id="type" name="type" required
                            ...
                            <option
                                value="percentage" {{ old('type', $discount->type) == 'percentage' ? 'selected' : '' }}>
                                Percentage (%)
                            </option>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" id="value" name="value"
                               value="{{ old('value', $discount->value) }}" placeholder="0.00" required
                               class="block w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Enter discount description..."
                              class="block w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors">{{ old('description', $discount->description) }}</textarea>
                </div>
            ...
            <div class="flex items-center gap-3">
                <button type="submit"
                    ...
                    <i data-lucide="save" class="w-4 h-4"></i> Update Discount
                </button>
                <a href="{{ route('finance.discounts.index') }}"
