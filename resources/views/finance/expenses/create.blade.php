...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="font-heading text-xl font-bold text-gray-900">Submit Expense</h1>
        <nav class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
            ...
            <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
            <span class="text-gray-900">Submit Expense</span>
        </nav>
    </div>
    ...
    <div class="bg-white rounded-card shadow-card border border-gray-200 p-6 max-w-4xl">
        <form action="{{ route('finance.expenses.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <select id="vendor_id" name="vendor_id"
                        ...
                        @foreach($vendors as $vendor)
                            <option
                                value="{{ $vendor->id }}" {{ old('vendor_id') == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mb-8">
                <label for="receipt" class="block text-sm font-medium text-gray-700 mb-1.5">Receipt Attachment
                    (Optional)</label>
