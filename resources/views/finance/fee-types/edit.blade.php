...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="font-heading text-xl font-bold text-gray-900">Edit Fee Type: {{ $fee_type->name }}</h1>
        <nav class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
            ...
            <a href="{{ route('finance.fee-types.index') }}" class="hover:text-indigo-600 transition-colors">Fee
                Types</a>
            <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
            <span class="text-gray-900">Edit Fee Type</span>
        </nav>
    </div>
    ...
    <div class="bg-white rounded-card shadow-card border border-gray-200 p-6 max-w-2xl">
        <form action="{{ route('finance.fee-types.update', $fee_type->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4 mb-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $fee_type->name) }}"
                ...
                <div>
                    <label for="code" class="block text-sm font-medium text-gray-700 mb-1.5">Code <sup
                            class="text-indigo-500">*</sup></label>
                    <input type="text" id="code" name="code" value="{{ old('code', $fee_type->code) }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Enter fee type description..."
                              class="block w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-colors">{{ old('description', $fee_type->description) }}</textarea>
                </div>

                <div class="flex items-center gap-6">
                    <div class="flex items-center">
                        <input id="recurring" name="recurring" type="checkbox" value="1"
                               {{ old('recurring', $fee_type->recurring) ? 'checked' : '' }}
                    ...
                    <div class="flex items-center">
                        <input id="active" name="active" type="checkbox" value="1"
                               {{ old('active', $fee_type->active) ? 'checked' : '' }}
                               class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
            ...
            <div class="flex items-center gap-3">
                <button type="submit"
                    ...
                    <i data-lucide="save" class="w-4 h-4"></i> Update Fee Type
                </button>
