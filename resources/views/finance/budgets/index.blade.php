...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="font-heading text-xl font-bold text-gray-900">Budgets</h1>
            <nav class="flex items-center gap-1.5 mt-1 text-sm text-gray-500">
                ...
                <i data-lucide="chevron-right" class="w-3.5 h-3.5"></i>
                <span class="text-gray-900">Budgets</span>
            </nav>
        </div>
        @can('finance.budget.manage')
            <a href="{{ route('finance.budgets.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm">
                <i data-lucide="plus" class="w-4 h-4"></i> Create Annual Budget
            </a>
        @endcan
    </div>
